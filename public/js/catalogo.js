...
async function carregarProdutosDoBackend() {
  try {
    ...
    let produtosBackend = [];
    if (resposta.produtos) produtosBackend = resposta.produtos;
    else if (Array.isArray(resposta)) produtosBackend = resposta;

    const produtosFormatados = produtosBackend.map((p, index) => ({
      id: 1000 + index, // ← IDs numéricos únicos
      nome: p.nome,
      preco: Number(p.preco),
      precoOriginal: Number(p.precoOriginal || p.preco),
      categoria: p.categoria || 'outros',
      imagem: p.imagem || 'img/placeholder.png',
      emOferta: !!p.emOferta,
      estoque: p.estoque || 10
    }));

    // Junta tudo
    produtosAtivos = [...produtos, ...produtosFormatados];
    window.todosProdutosCarrinho = [...produtosAtivos];

    console.log("Total de produtos carregados:", produtosAtivos.length);

  } catch (error) {
    console.error("Falha ao carregar backend:", error);
    // Continua com os fixos se o backend cair
    produtosAtivos = [...produtos];
    window.todosProdutosCarrinho = [...produtos];
  }
}
...
function aplicarFiltros() {
  ...
  let produtosFiltrados = produtosAtivos.filter(produto => {
